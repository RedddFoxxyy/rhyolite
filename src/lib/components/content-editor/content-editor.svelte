<script lang="ts">
		...
		editor = createEditor({
			extensions: [
				StarterKit.configure({
					...
					blockquote: {
						HTMLAttributes: {
							class: "border-l-2 border-subtext2 m-1 p-2 w-fit text-text text-base bg-base/60 font-normal not-italic leading-none before:content-none after:content-none"
						}
					},
					paragraph: false,
					code: {
				...
				TaskItem.configure({
					HTMLAttributes: {
						class: "flex flex-row gap-2 leading-none [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 "
					},
					nested: true
				}),
				TaskList.configure({
			...
			editorProps: {
				attributes: {
					class: "format text-text text-base focus:outline-none format-blue max-w-none"
				}
			},
	...
	const setTextSize = (event: Event) => {
		const fontSize: string | null = (event.target as HTMLInputElement).getAttribute(
			"data-text-size"
		);
	...
	const onClickColorHex = (event: Event) => {
		const selectedColor: string | null = (event.target as HTMLInputElement).getAttribute(
			"data-hex-color"
		);
		// Apply the selected color to the selected text
	...
	const onSelectFontFamily = (event: Event) => {
		const fontFamily: string | null = (event.target as HTMLInputElement).getAttribute(
			"data-font-family"
		);
	...
	const onSelectHeading = (event: Event) => {
		const headingLevel: string | null = (event.target as HTMLInputElement).getAttribute(
			"data-heading-level"
		);
		if (headingLevel) {
